...
Version 1.4 retains the P02 transition and ending fixtures. Separate catch-opening and clamp-collection effects are preserved. The backlog includes the revised opening and observation criteria; its numeric effort ranges still come from v1.1 and require review after the paper trial. [Document 12](../12-gameplay-and-playtest-plan.md) is the detailed design companion; [document 13](../13-paper-playtest-pack.md) supplies staged cards for the current P02 contract. The v1.4 friendship revision adds chapter acceptance criteria and retires two unshipped local dialogue names; it adds no ending gate or runtime system.
